...
func TestAdapter_ProjectInit_ValidJSON(t *testing.T) {
	projectInitJSON := domain.ProjectInitResult{
		Description: "A git helper that generates conventional commits",
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Format != "json" {
			t.Errorf("Format: got %q, want %q", req.Format, "json")
		}

		// Verify prompt contains directory tree placeholder content
		if len(req.Messages) < 2 {
			t.Fatalf("messages: got %d, want at least 2 (system /no_think + user)", len(req.Messages))
		}
		if req.Messages[1].Role != "user" {
			t.Errorf("second message role: got %q, want %q", req.Messages[1].Role, "user")
		}
		// The prompt should contain the repoRoot path since we pass it as DirectoryTree
		if !strings.Contains(req.Messages[1].Content, "/tmp/myproject") {
			t.Errorf("user prompt should contain the directory tree; got:\n%s", req.Messages[1].Content[:min(200, len(req.Messages[1].Content))])
		}

		w.Header().Set("Content-Type", "application/json")
		w.Write(chatCompletionResponse(mockJSONResponse(t, projectInitJSON)))
	}))
	defer server.Close()

	adapter := newTestAdapter(server)
	config, err := adapter.ProjectInit("/tmp/myproject")
	if err != nil {
		t.Fatalf("ProjectInit() error: %v", err)
	}
	if config == nil {
		t.Fatal("ProjectInit() returned nil config")
	}
	if config.Description != "A git helper that generates conventional commits" {
		t.Errorf("Description = %q, want %q", config.Description, "A git helper that generates conventional commits")
	}
	if len(config.Areas) != 2 {
		t.Fatalf("Areas count = %d, want 2", len(config.Areas))
	}
	if len(config.Areas["core"]) != 2 {
		t.Errorf("Areas[core] len = %d, want 2", len(config.Areas["core"]))
	}
}

func TestAdapter_ProjectInit_InvalidJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Write(chatCompletionResponse("not valid json at all"))
	}))
	...
	adapter := newTestAdapter(server)
	_, err := adapter.ProjectInit("/tmp/myproject")
	if err == nil {
		t.Fatal("expected error for invalid JSON, got nil")
	}
	if !errors.Is(err, ErrInvalidJSON) {
...
func TestAdapter_ProjectInit_EmptyResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Write(chatCompletionResponse(""))
	}))
...
func TestAdapter_ProjectInit_UsesProjectInitPrompt(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var req ChatRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			t.Errorf("failed to decode request: %v", err)
		...
		// Verify the prompt uses the project_init template by checking for key phrases
		userContent := req.Messages[1].Content
		if !strings.Contains(userContent, "Analyze the following project directory structure") {
			t.Errorf("prompt should contain project_init template text; got:\n%s", userContent[:min(300, len(userContent))])
		}
		if !strings.Contains(userContent, "description") {
			t.Errorf("prompt should ask for 'description'; got:\n%s", userContent[:min(300, len(userContent))])
		}
		if !strings.Contains(userContent, "areas") {
			t.Errorf("prompt should ask for 'areas'; got:\n%s", userContent[:min(300, len(userContent))])
		}

		projectInitJSON := domain.ProjectInitResult{Description: "test project"}
		w.Header().Set("Content-Type", "application/json")
		w.Write(chatCompletionResponse(mockJSONResponse(t, projectInitJSON)))
	}))
	defer server.Close()
...
func TestAdapter_ProjectInit_EmptyAreas(t *testing.T) {
	// Triangulate: LLM returns valid JSON with description but no areas
	projectInitJSON := domain.ProjectInitResult{
	...
	if config.Description != "A simple project" {
		t.Errorf("Description = %q, want %q", config.Description, "A simple project")
	}
	if len(config.Areas) != 0 {
		t.Errorf("Areas count = %d, want 0", len(config.Areas))
	}
}

func TestAdapter_ProjectInit_Params(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var req ChatRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			t.Errorf("failed to decode request: %v", err)
		...
		if req.Temperature == nil {
			t.Fatal("ProjectInit: temperature is nil, want non-nil pointer to 0.1")
		}
		if *req.Temperature != 0.1 {
			t.Errorf("ProjectInit temperature: got %f, want 0.1", *req.Temperature)
		}
		if req.MaxTokens != 512 {
			t.Errorf("ProjectInit maxTokens: got %d, want 512", req.MaxTokens)
		}

		projectInitJSON := domain.ProjectInitResult{Description: "test"}
		w.Header().Set("Content-Type", "application/json")
		w.Write(chatCompletionResponse(mockJSONResponse(t, projectInitJSON)))
	}))
	defer server.Close()
