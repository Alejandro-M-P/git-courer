...
const (
	...
	DescSession = `Isolated git worktree + branch per agent for parallel work. ` + "`start`" + ` creates a ` + "`{id}`" + ` branch (atomic ` + "`git update-ref`" + `) and a linked worktree at ` + "`../git-courer-worktrees/{id}/`" + `, persisting session metadata. ` + "`finish`" + ` loads a session, runs preview validation (uncommitted changes, test command, dry-run merge conflict), no merge de branch into original repo , then cleans up the worktree. ` + "`status`" + ` returns the current session state (active | finished | cleanup_failed). ` + "`discard`" + ` removes a session— requires ` + "`confirmed=true`" + `. The ` + "`agent`" + ` and ` + "`goal`" + ` parameters are required for ` + "`start`" + `. ` + "`branch`" + ` is optional for ` + "`start`" + ` — when provided, uses that exact branch name (fails if exists); when omitted, derives from goal slug. ` + "`session_id`" + ` is required for finish/status/discard.`
)
