...
func registerTools(s *server.MCPServer, srv *Server) {
	...
	coreHandler := core.NewHandler(srv.git, srv.commitSvc, srv.reviewWorkflow, srv.llm, provider, s)
	...
	workDir := "."
	utilityHandler := utility.NewHandler(srv.git, srv.cfg, workDir, srv.releaseSvc, srv.mcpServer)
