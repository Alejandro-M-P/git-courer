...
func TestLogResultJSON_SemanticKeys(t *testing.T) {
	res := LogResult{
		Commits: []CommitEntry{
			{Hash: "abc", Message: "test", Author: "me", Date: "today"},
		},
		TotalCommits: 1,
		Returned:     1,
	}

	result := logResultJSON(res)

	var parsed map[string]interface{}
	json.Unmarshal([]byte(result), &parsed)

	if _, ok := parsed["commits"]; !ok {
		t.Error("Missing 'commits' key")
	}
	if _, ok := parsed["c"]; ok {
		t.Error("Found legacy 'c' key")
	}

	commits := parsed["commits"].([]interface{})
	c := commits[0].(map[string]interface{})
	if _, ok := c["hash"]; !ok {
		t.Error("Missing 'hash' key in commit")
	}
}

func TestBlameResultJSON_SemanticKeys(t *testing.T) {
	lines := []domain.BlameLine{
		{Line: 1, Author: "me", Hash: "abc"},
	}

	result := blameResultJSON("file.go", lines, 10, 0)

	var parsed map[string]interface{}
	json.Unmarshal([]byte(result), &parsed)

	if _, ok := parsed["file"]; !ok {
		t.Error("Missing 'file' key")
	}
	if _, ok := parsed["fl"]; ok {
		t.Error("Found legacy 'fl' key")
	}

	lItems := parsed["lines"].([]interface{})
	l := lItems[0].(map[string]interface{})
	if _, ok := l["line_number"]; !ok {
		t.Error("Missing 'line_number' key")
	}
}
