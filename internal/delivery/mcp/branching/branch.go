...
func (h *Handler) HandleBranch(_ context.Context, req mcpgo.CallToolRequest) (*mcpgo.CallToolResult, error) {
	...
	if result, err := shared.ValidateKnownParams(params, []string{"command", "branch_name", "new_branch_name", "remote_name", "force", "confirmed"}); result != nil || err != nil {
		return result, err
	}
	...
	validBranchCommands := []string{"CREATE", "DELETE", "RENAME", "REMOTE_DELETE", "SET_UPSTREAM", "UNSET_UPSTREAM", "SWITCH"}
