...
func Register(s *server.MCPServer, h Handlers) {
	...
	s.AddTool(
		...
		mcpgo.WithString("target_paths", mcpgo.Required(), mcpgo.Description("Space-separated file paths to stage before generating the preview plan. Use \".\" to stage all changes.")),
