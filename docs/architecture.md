...
- **AI Toggle**: Non-AI execution mode when `llm.enabled` is `false` to run offline without LLM requirements.
...
│   │   ├── github/         # API client for PR integration (optional)
...
│   ├── data/               # Static/embedded data (languages.json)
│   ├── delivery/
...
│   │   ├── filters/        # Path/file filtering
│   │   └── secrets/        # Regex & magic bytes secret matching rules
│   ├── installer/          # CLI installer & client configuration setup
...
│   └── workflow/           # Main business workflows (CommitService, ReleaseService)
...
Before staging or committing any changes, the security orchestrator checks the working tree and staged diffs in memory using 6 cumulative defense layers:

1. **Binary File Block**: Magic bytes check (`IsBinary`) on files + AI-powered file noise analysis (`AuditBinaryContent`) to detect obfuscated payloads disguised as plain text.
2. **Folder Blacklist**: Rejects tracking in sensitive directories (e.g. `vendor/`, `.git/`, `.github/`, `test/`).
3. **Name Blacklist**: Blocks known sensitive files (e.g. `.env`, `id_rsa`, `credentials.json`, `auth.key`).
4. **Static Analysis**: Runs external detectors like `trufflehog` or `gosec` on files if they are installed on the system.
5. **Regex Search**: Matches files and staged diff lines against a library of regular expressions targeting common secret formats (e.g. Stripe keys, AWS credentials).
6. **LLM Verification**: A paranoid AI auditor agent (`VerifySecrets`) parses the diff to filter false positives. If the LLM validates that the match is not a secret, it can override low-confidence regex flags.
